...
data_dir = "/mnt/pengber/data/Reuters-21578/data_BalancedLossNLP/"
proc_data_dir = data_dir + "proc_data"
# source_dir = './'
prefix = 'reuters'
suffix = 'rand123'
...
data_train = pickle.load(open(os.path.join(proc_data_dir, 'data_train.' + suffix), 'rb'))
data_val = pickle.load(open(os.path.join(proc_data_dir, 'data_val.' + suffix), 'rb'))
labels_ref = pickle.load(open(os.path.join(proc_data_dir, 'labels_ref.' + suffix), 'rb'))
class_freq = pickle.load(open(os.path.join(proc_data_dir, 'class_freq.' + suffix), 'rb'))
train_num = pickle.load(open(os.path.join(proc_data_dir, 'train_num.' + suffix), 'rb'))
...
model = nn.DataParallel(
    AutoModelForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=num_labels)).to(device)
...
for epoch in trange(epochs, desc="Epoch"):
    ...
    if f1_results[best_f1_idx] > (best_f1_for_epoch * 0.995):
        ...
        torch.save(model.state_dict(), os.path.join(model_dir, '_'.join(
            [prefix, model_name, loss_func_name, suffix, 'epoch']) + str(epoch + 1) + 'para'))
    else:
    ...
    with open(os.path.join(log_dir,
                           '_'.join([prefix, model_name, loss_func_name, suffix, 'epoch']) + str(epoch + 1) + '.json'),
              'w') as f:
        d = {}
    ...
    open(os.path.join(log_dir,
                      '_'.join([prefix, model_name, loss_func_name, suffix, 'epoch']) + str(epoch + 1) + '.tmp'),
         'w').write('%s %s' % (micro_thresholds[best_f1_idx], f1_results[best_f1_idx]))
